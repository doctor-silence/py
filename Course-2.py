...
#Напишите программу, которая принимает из аргументов командной строки три параметра: год, номер месяца (1 - январь и т.д.), 
# а также новое значение для этого месяца.
...
budget[year][month] = price
summa = sum(budget[year]) * 1000
summa = "{0:,}".format(summa).replace(",", ' ')
print(summa)
...
#Напишите программу, которая принимает из аргументов командной строки данные в следующем виде: первым аргументом идет индекс категории в списке products, 
# а далее следует список товаров. Программа должна расширить вложенный список, который находится по переданному индексу,
# товарами, которые размещены в остальных аргументах. В конце нужно вывести получившийся список.

index = 1
product = ['свинина, купаты']
products = [
    ["молоко", "кефир"],  # молочка
    ["котлеты", "курица", "говядина"]  # мясо
]

products[index].extend(product)
...
#Программа ниже принимает из аргументов командной строки список автомобильных марок, а затем сортирует его в обратном порядке и выводит результат. 
# Проблема в том, что код не работает. Исправьте все ошибки.


cars = ['Toyota', 'Honda', 'BMW', 'Kia']

# Сортируем в обратном порядке.
cars.sort(reverse=True, key=str.upper)

# Выводим результат.
print(cars)


#Напишите программу, которая принимает из аргументов командной строки список чисел, а затем выводит их в порядке убывания. 
# Учитывайте, что все данные из аргументов командной строки в программу поступают в виде строк, а нам нужно получить сортировку с позиции чисел.

number = ['120', '56', '3', '17', '100', '9']

number.sort(key=int, reverse=True)

print(number)



#Во многих видах спорта на соревнованиях работает следующая схема выставления оценок. Сперва убирается одна максимальная и минимальная оценка, 
# а уже после рассчитывается среднее значение из оставшихся. Это позволяет исключить заниженные и завышенные оценки.

#Напишите программу, которая принимает произвольное количество оценок, затем исключает минимум и максимум, а после рассчитывает среднее из оставшихся. 
# Программа должна вывести список оригинальных оценок, а также среднее с двумя знаками после десятичной точки.


votes_1 = ['10', '9', '8', '7', '9', '10', '9', '9', '8', '9']
votes = [10, 9, 8, 7, 9, 10, 9, 9, 8, 9]

votes = list(map(int, votes))
votes_min = min(votes)
votes_max = max(votes)
votes.remove(votes_max)
votes.remove(votes_min)
votes = sum(votes) / len(votes)
votes_1 = list(map(int, votes_1))

print(votes_1,'{:.2f}'.format(votes))